...
namespace Ideastrike
{
    ...
    {
        private static string formsAuthenticationCookieName = "_ideastrike";
        ...
        private static Func<NancyContext, Response> GetLoadAuthenticationHook(FormsAuthenticationConfiguration configuration)
        {
            ...
            return context =>
            {
                var userID = GetAuthenticatedUserFromCookie(context, configuration);

                if (userID != Guid.Empty)
                {
                    context.CurrentUser =
                        configuration.UserMapper.GetUserFromIdentifier(userID);
                }
        ...
        private static Action<NancyContext> GetRedirectToLoginHook(FormsAuthenticationConfiguration configuration)
        {
            return context =>
            {
                ...
                {
                    context.Response = context.GetRedirect(configuration.RedirectUrl);
                }
            };
        }
        ...
        private static Guid GetAuthenticatedUserFromCookie(NancyContext context, FormsAuthenticationConfiguration configuration)
        {
            ...
            Guid returnUserID;
            if (String.IsNullOrEmpty(cookieValue) || !Guid.TryParse(cookieValue, out returnUserID))
            {
                return Guid.Empty;
            }
